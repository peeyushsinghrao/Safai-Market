...
import { useGetPurchase, useFinalizePurchase, getListPurchasesQueryKey, getGetPurchaseQueryKey } from "@workspace/api-client-react";
...
import { FileText, Calendar, Box, Package, CheckCircle } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

export default function PurchaseEdit() {
  const [, params] = useRoute("/purchases/:id/edit");
  ...
  const finalizeMutation = useFinalizePurchase();
  ...
  const handleFinalize = () => {
    finalizeMutation.mutate({ id: purchaseId }, {
      onSuccess: () => {
        toast({ title: "Purchase Finalized", description: "Stock and accounts updated successfully." });
        queryClient.invalidateQueries({ queryKey: getListPurchasesQueryKey() });
        queryClient.invalidateQueries({ queryKey: getGetPurchaseQueryKey(purchaseId) });
        setLocation(`/purchases/${purchaseId}`);
      },
      onError: (err: any) => {
        toast({ title: "Error", description: err.message || "Failed to finalize.", variant: "destructive" });
      }
    });
  };
  ...
  return (
    <div className="flex flex-col min-h-full bg-slate-50 font-sans">
      <PageHeader title={`Draft: ${purchase.purchaseNumber}`} backTo="/purchases" />
      
      <div className="p-4 space-y-4 pb-24">
        <div className="bg-amber-50/50 rounded-2xl border border-amber-200 shadow-sm p-5">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="text-[12px] px-3 py-1 bg-amber-100 text-amber-800 border border-amber-200 rounded-xl font-bold uppercase">
              {purchase.status?.toUpperCase()} DRAFT
            </span>
          </div>
          
          <div className="grid grid-cols-2 gap-4 mt-6 p-4 bg-white/60 rounded-2xl border border-amber-100">
            <div>
              <p className="text-[11px] text-slate-500 uppercase tracking-wider font-bold mb-1 flex items-center gap-1">
                <FileText className="w-3.5 h-3.5" />
                Invoice Ref
              </p>
              <p className="font-bold text-[15px] text-slate-800">{(purchase as any).invoiceNumber || purchase.invoiceRef || 'N/A'}</p>
            </div>
            <div>
              <p className="text-[11px] text-slate-500 uppercase tracking-wider font-bold mb-1 flex items-center gap-1">
        ...
        {purchase.status === 'draft' && (
          <div className="mt-8 pt-4">
            <Button
              className="w-full h-14 text-[16px] font-bold rounded-2xl shadow-sm active-elevate transition-transform bg-primary hover:bg-primary/90 text-white"
              onClick={handleFinalize}
              disabled={finalizeMutation.isPending}
            >
              <CheckCircle className="w-5 h-5 mr-2" />
              {finalizeMutation.isPending ? "Finalizing..." : "Finalize Purchase (Add to Stock)"}
            </Button>
            <p className="text-xs text-center text-slate-500 font-medium mt-3">
              Finalizing will update product stock and supplier ledger.
            </p>
          </div>
        )}
      </div>
